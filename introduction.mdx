...
<AccordionGroup>
  <Accordion title="Agents Experimentation">
    Supervise and evaluate agentic systems during experimentation to quickly iterate on your agent architecture.
    - **Tool Call Supervision**: Fix failure modes by supervising tool calls. Supervisors can discover failure modes and improve the agent's behaviour.
  ...
  <Accordion title="Agents in Production">
    Deploy your agent to production with Sentinel's supervisors that are monitoring for undesired behaviour, can intervene to correct the agent, terminate the agent or escalate to a human reviewer.
    ...
    - **Evaluations**: Automatically create evaluations from your agent's execution logs and supervisors interventions.
 </Accordion>
</AccordionGroup>
...
<CardGroup cols={3}>
  ...
  <Card title="Deterministic Supervisor" icon="code">
    <p>
      A deterministic supervisor uses a predefined set of rules to determine the approval decision. This is useful for simple approval processes that are known ahead of time, such as whitelisting emails, commands, URLs, enforcing Regex patterns, etc.
    </p>
  </Card>
</CardGroup>
