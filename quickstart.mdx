...
Sentinel can be used with any agent built in Python using the `asteroid-sdk` Python SDK with Sentinel server and web interface which is hosted locally using Docker.
...
<Steps>
  ...
  <Step title="Clone the repository">
    ...
    ```bash
    git clone https://github.com/asteroidai/sentinel && cd sentinel
    ```
  </Step>
...
- **Built-in Supervisors**: We also use `llm_supervisor` and `human_supervisor` from the `asteroid-sdk`.
