...
Inspect AI is a powerful open-source framework for large language model evaluations. This guide will help you get started with running examples, editing configurations, and understanding the main functionalities of using asteroid_ai's approvers together with Inspect.
...
To get started, ensure that `inspect-ai` and `asteroid_ai` are installed in your Python environment:

````bash
pip install inspect-ai asteroid_ai --upgrade
````
...
We follow the Inspect AI format for the approval configuration. Our approvers are in the `asteroid_ai` package. You can install the `asteroid_ai` package to get these approvers:

````bash
pip install asteroid_ai
````

Then, you can reference the approvers in the approval configuration using names like `asteroid_ai/human_approver`.
...
approvers:
  - name: asteroid_ai/human_approver
    tools: "*"
...
approvers:
  - name: asteroid_ai/bash_approver
    tools: "*bash*"
  ...
  - name: asteroid_ai/python_approver
    tools: "*python*"
