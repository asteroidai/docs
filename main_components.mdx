...
When working with OpenAI's chat models, you need to update the conversation history in the `SupervisionContext` so Asteroid can keep track of messages and facilitate supervision.

Here's an example of how you can update the messages using the `SupervisionContext`:

```python
from asteroid_sdk.supervision.config import get_supervision_context
...
from uuid import UUID
from asteroid_sdk.supervision.supervisors import Supervisor
from asteroid_sdk.supervision.config import SupervisionDecision, SupervisionDecisionType, SupervisionContext
>>>>>>> Stashed changes
