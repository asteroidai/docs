...
description: 'Main components of the Asteroid AI package'
---

# Main Components of Asteroid AI Package

The Asteroid AI package provides tools to build robust AI applications with supervision and evaluation capabilities. By integrating projects, tasks, runs, supervision functions, the `@supervise` decorator, and custom supervisors, you can create AI systems that are controllable, testable, and can involve human oversight.

This section provides an overview of the main components of the Asteroid AI package, along with code examples to illustrate their usage.

## Projects, Tasks, and Runs

In the Asteroid AI framework, a **Project** is a container for tasks, which in turn contain runs. This hierarchy helps organize and track experiments and deployments.
...
To register a project, task, and run, you can use the `register_project`, `register_task`, and `create_run` functions provided by the `asteroid_ai` package.

Here's how you can register all three in one Python snippet:

```python
from asteroid_ai.api import register_project, register_task, create_run

# Specify the backend URL for Asteroid AI (assuming it's running locally)
asteroid_ai_backend_url = "http://localhost:8080"

# Register a project
project_id = register_project(
    project_name="My AI Assistant",
    asteroid_ai_backend_url=asteroid_ai_backend_url
)
...
This code snippet demonstrates how to set up the project structure in Asteroid AI, allowing you to track and manage your AI application's executions.
...
```python
from asteroid_ai.supervision import supervise, MockPolicy
...
```python
from asteroid_ai.supervision import supervise, human_supervisor, llm_supervisor
...
from uuid import UUID
from asteroid_ai.supervision.config import SupervisionContext, SupervisionDecision
...
from uuid import UUID
from asteroid_ai.supervision.supervisors import Supervisor
from asteroid_ai.supervision.config import SupervisionDecision, SupervisionDecisionType, SupervisionContext
...
```python
from asteroid_ai.supervision.supervisors import human_supervisor
...
When the `sensitive_action` function is called, it will pause execution until a human reviewer approves or rejects the action via the Asteroid AI supervision interface.
...
The Asteroid AI package provides the tools you need to add supervision and evaluation capabilities to your AI systems, facilitating better control and testing.
