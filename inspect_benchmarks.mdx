...
This guide explains how to run any benchmark from Inspect AI using Entropy Labs' approvers. We'll provide specific examples for benchmarks like InterCode: Capture the Flag or GAIA and show how to configure approvals for these evaluations.
...
```
This command will run the GDM In-House CTF evaluation with 10 samples and use the approval configuration specified in the `approval.yaml` file.
</Step>
...
```
This command will run the Intercode CTF evaluation with 10 samples and use the approval configuration specified in the `approval.yaml` file.
</Step>
...
```bash
inspect eval inspect_evals/gaia --model openai/gpt-4o --limit 10 --approval approval.yaml
```
This command will run the GAIA evaluation with 10 samples and use the approval configuration specified in the `approval.yaml` file.
</Step>
